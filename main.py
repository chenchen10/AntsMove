        ...
        if ant.state == Ant.STATE_IDLE:
            ...
            if all_targets:
                if is_player:
                    ...
                    # AI也检查昆虫是否更近
                    if alive_creatures:
                        nearest_creature = min(alive_creatures, key=lambda c: math.hypot(c.x - ant.x, c.y - ant.y))
                        if ant.target_sweet is None or math.hypot(nearest_creature.x - ant.x, nearest_creature.y - ant.y) < math.hypot(ant.target_sweet.x - ant.x, ant.target_sweet.y - ant.y):
                            ant.target_sweet = nearest_creature
